...
export class P3 {
    ...
    updateCategoriyList(categories) {
        for (const category of categories) {
            ...
            exp.push(...lsex, ...ksex, ...coex, ...edex)
            exp = _.uniqWith(exp, _.isEqual)

            category['total_similarity'] = total_similarity
            category['total_explanations'] = exp
            category['color'] = this.colorConverter(Number(total_similarity))
    ...
    computeLabelSimilarities(labels, keyword) {
        ...
        for (let label of labels) {
            ...
            exp.push(...label.similarities[0]['explanation'], ...label.similarities[1]['explanation'], ...label.similarities[2]['explanation'], ...label.similarities[3]['explanation'])
            exp = _.uniqWith(exp, _.isEqual)

            label['total_explanations'] = exp
            label['color'] = this.colorConverter(Number(label['total_similarity']))
        }
